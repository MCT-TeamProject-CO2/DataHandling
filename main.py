...
if __name__ == "__main__":
    try:
        # set the on message function for the mqtt client
        client.on_message = on_message
        
        # connect to the client and sunscribe to the topic
        client.connect(host_address, topic, keep_alive=60)
        
        # keep looping and keep reading data from the broker
        client.loop_forever(timeout=0.5)
    except KeyboardInterrupt:
        # if manually stopped through a keyboardinterupt, give general message
        print("\nManually stopped")
